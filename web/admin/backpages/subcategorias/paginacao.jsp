...
<%
    ...
    if (request.getParameterMap().containsKey("pesquisa") &&
            !request.getParameter("pesquisa").equals("")) {

        pesquisa = request.getParameter("pesquisa");
        where = " WHERE subcategoria ";
        query = " LIKE '%" + pesquisa + "%'";
        estaativourl = request.getParameter("searching-option");
        if (request.getParameter("searching-option").equals("Inativo")) {
            estaativoquery = " AND ativo_subcategoria = 'N' ";
        } else if (request.getParameter("searching-option").equals("Subcategoria")) {
            estaativoquery = "";
        } else {
            estaativoquery = " AND ativo_subcategoria = 'S' ";
        }
    ...
    //instacia CRUD categoria
    CategoriasDAO catdao = new CategoriasDAO();

    //instancia de CRUD da subcategoria
    SubCategoriasDAO subcatdao = new SubCategoriasDAO();

    //recebe dados da listagem de categoria do CRUD especificando a query sql
    List<Subcategoria> listainteira = subcatdao.read("SELECT id_subcategoria, categoria.id_categoria, " +
            "subcategoria, categoria, ativo_subcategoria FROM categoria INNER JOIN subcategoria ON " +
            "categoria.id_categoria = subcategoria.id_categoria "
            + where + query + estaativoquery + ";");

    List<Categoria> listanomecategoria = catdao.readnamecat();
    ...
    if (ordem == 0) {
        ...
        listainteira = subcatdao.read("SELECT id_subcategoria, categoria.id_categoria, " +
                "subcategoria, categoria, ativo_subcategoria FROM categoria INNER JOIN " +
                "subcategoria ON categoria.id_categoria = subcategoria.id_categoria " +
                where + query + estaativoquery + " LIMIT " + inicio + ", " + LPP + ";");

        proximo = ordem + 1;

        url_proximo = "backpages/subcategorias/model.jsp?acao=visualizar&ordem="
                + proximo + "&pesquisa=" + pesquisa + "&searching-option=" + estaativourl;
    }
    ...
    if (ordem > 0) {
        ...
        proximo = ordem + 1;
        url_proximo = "backpages/subcategorias/model.jsp?acao=visualizar&ordem=" + proximo +
                "&pesquisa=" + pesquisa + "&searching-option=" + estaativourl;

        anterior = ordem - 1;

        url_voltar = "backpages/subcategorias/model.jsp?acao=visualizar&ordem=" +
                anterior + "&pesquisa=" + pesquisa + "&searching-option=" + estaativourl;
    }
    ...
    session.removeAttribute("listasubcategoria");
    session.removeAttribute("subcatindices");
    session.removeAttribute("listanomescategorias");
    session.setAttribute("listasubcategoria", listainteira);
    session.setAttribute("subcatindices", pag);
    session.setAttribute("listanomescategorias",listanomecategoria);
%>