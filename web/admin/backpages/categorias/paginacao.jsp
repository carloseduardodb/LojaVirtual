...
<%
    ...
    if (request.getParameterMap().containsKey("pesquisa") && !request.getParameter("pesquisa").equals("")) {
        ...
        if (request.getParameter("searching-option").equals("Inativo")) {
            estaativoquery = " AND ativo_categoria = 'N' ";
        } else if (request.getParameter("searching-option").equals("Categoria")) {
            estaativoquery = "";
        } else {
            estaativoquery = " AND ativo_categoria = 'S' ";
        }
    }
    ...
    List<Categoria> listainteira = catdao.read("SELECT * FROM categoria " + where + query + estaativoquery + ";");
    ...
    if (ordem == 0) {
        ...
        listainteira = catdao.read("SELECT * FROM categoria " + where + query + estaativoquery + " LIMIT " + inicio + ", " + LPP + ";");
        proximo = ordem + 1;
        url_proximo = "backpages/categorias/model.jsp?acao=visualizar&ordem=" + proximo + "&pesquisa=" + pesquisa + "&searching-option=" + estaativourl;
    }
    ...
    if (ordem > 0) {
        ...
        anterior = ordem - 1;
        url_voltar = "backpages/categorias/model.jsp?acao=visualizar&ordem=" + anterior + "&pesquisa=" + pesquisa + "&searching-option=" + estaativourl;
    }
